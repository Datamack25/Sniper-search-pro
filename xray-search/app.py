...
def build_query(params: dict) -> dict:
    ...
    # Param 1: Keywords (quoted, joined with OR)
    kw_raw = params.get("keywords", "")
    if kw_raw:
        kws = [k.strip() for k in kw_raw.replace(",", " ").split() if k.strip()]
    ...
    variants["🔵 Recherche générale"] = f"{base_no_excl} {' '.join(DEFAULT_EXCLUSIONS[:5])}"

    buy_intent = '+acheter OR +prix OR +tarif OR +service'
    variants["🔴 Achat / Service"] = f"{base_no_excl} {buy_intent} {' '.join(DEFAULT_EXCLUSIONS[:4])}"

    app_intent = '+application OR +appli OR +télécharger'
    variants["🟢 Application"] = f"{base_no_excl} {app_intent} -gratuit -PDF"

    pro_intent = '+professionnel OR +expert OR +consultant'
    variants["🟡 Professionnel"] = f"{base_no_excl} {pro_intent} -forum -wiki"
...
with tab1:
    ...
    with col_left:
        ...
        keywords = st.text_input(
            "① Mots-clés (séparés par virgule ou espace)",
            placeholder="ex: python, django, fastapi",
            help="Chaque mot sera entouré de guillemets et joint avec OR",
        )
    ...
    with col_right:
        ...
        if generate or any([keywords, job_title, location, employer]):
            ...
            if not any([keywords, job_title, location, employer]):
                ...
                # Variants
                st.markdown('<div class="section-header">🎨 Variantes par objectif</div>', unsafe_allow_html=True)
                for label, variant_q in result["variants"].items():
                    variant_url = f"https://www.google.com/search?q={urllib.parse.quote(variant_q)}"
                    st.markdown(f"""
                    <div class="variant-box">
                      <div class="variant-label">{label}</div>
                      {variant_q}
                      <br><a href="{variant_url}" target="_blank" style="color:#7b61ff;font-size:0.75rem;text-decoration:none;font-family:'Space Grotesk',sans-serif;">↗ Ouvrir dans Google</a>
                    </div>
                    """, unsafe_allow_html=True)
